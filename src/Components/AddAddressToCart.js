...
import "../Styles/AddAddressToCart.css";
...
function AddAdressToCart() {
    ...
    async function updateDataUser(data) {
        try {
            ...
            if(res.data.success){
                dispatch(reload())
                dispatch(setMessage({
                    message: "Se actualizó la direccion de envio",
                    success: true
                }))
  ...
  return (
    <div className="containerAddressCart">
      
      <form className="editProfileFormCart" ref={refData} onSubmit={handleSubmit}>
        <label>
          Direccion
        ...
        <label>
          Codigo Postal
          <input  type="number" placeholder="Codigo postal" name="zipCode" />
        </label>
        <label>
          Ciudad
        ...
        <button className="buttonBuyAddress" >Agregar Datos de envio</button>
      </form>
    </div>
  );
}

export default AddAdressToCart;
